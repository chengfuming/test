async function main(xky, args, xutil) {
  ...
  await setChuDong(xky, args, xutil, 3, args.user_position);
  await dengji(xky, args, xutil);
}

async function dengji(xky, args, xutil) {
  let aa = args.AllMassage.split(" ")
  let [ysbh, phone, passWord, nickName] = aa;
  await xky.clearContacts();
  let phonePeopleArr = await toLeadInsCon(xky, xutil, 5);
  await setChuDong(xky, args, xutil, 1, args.user_position);
  await setLocations(xky);
  let ccCode = await creatSlot(xky, "com.tencent.mm", 18);
  await elementClick(xky, "登录");
  await findElementIput(xky, "请填写手机号", phone);
  await elementClick(xky, "下一步");
  await findElementIput(xky, "请填写微信密码", passWord);
  await elementClick(xky, "登录");
  await slideWecatJigsaw(xky, "tcaptcha_drag_thumb");
  await findElementIput(xky, "请填写微信密码", passWord,2);
  await elementClick(xky, "登录");
  await elementClick(xky, "开始验证 ");
  await elementClick(xky, "扫二维码验证 ");
  if (args.setModule == "好友发验证") {
    await elementClick(xky, "手机不在身边或无法扫码？");
  ...
  let saveUser = await saveData(xky, xutil, args.AppID, "", args.depot, phone, passWord, phone, nickName, ysbh, args.user_position,extendData);
  xky.log(saveUser);
  await xky.toast("正在上传插槽信息");
  await xky.sleep(5000);
  await xky.killApp('com.tencent.mm');
  await xky.sleep(2 * 1000);
  await setChuDong(xky, args, xutil, 2, args.user_position);
  await xky.sleep(2 * 1000);
  await uplondWeFile(xky, "WeChat", "com.tencent.mm", ccCode);
