...
async function doModule(xky, args, xutil, maincheck, qqq) {
    await isAuthentication(xky, args, xutil, qqq);
...
//实名昵称更改
async function isAuthentiNick(xky, args, xutil, qqq) {
    ...
    if (yirenzheng) {
        ...
        if (smmmm < 0) {
            ...
            let nickSave = await saveData(xky, xutil, 1, qqq.data.data.id, qqq.data.data.depot, qqq.data.data.account, qqq.data.data.password, qqq.data.data.mobile, `${yigai["0"].text}mars`, qqq.data.data.rawid, qqq.data.data.city, extendData.cCode, extendData.addressbook);
            xky.log(nickSave);
        }
    } else {
        await findUseFunction(xky, "com.tencent.mm:id/d7b", "返回");
...
async function donwlondWeFile(xky, AppUsesName, bagName, backupname) {
    ...
    while (1) {
        await xky.adbShell(`curl ftp://192.168.2.152/data/${AppUsesName}/${backupname}.tar -u "admin:123456" -o /sdcard/xbak/${backupname}.tar`);
        await xky.sleep(2000);
