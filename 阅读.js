async function main(xky, args, xutil) {
  ...
  //设置VPN账号
  await chudong(xky, args, xutil, 4, args.user_position);
  //获取仓库
  let getPot = await getDepot(xky, xutil, 1);
  let depotArrObj = [];
  for (let i = 3; i < getPot.data.data.length; i++) {
...
async function repairError(xky, args, xutil, qqq) {
  while (1) {
    while (1) {
      ...
      let goodUser = await ytl(xky, "hqxx", "com.tencent.mm:id/qr", 1);//限时查找
      if (qqwe1.uiObjects != "") {
        await ytl(xky, "fdinp", "请填写微信密码", qqq.data.data.password);
        break;
      } else if (qqwe2.uiObjects != "") {
        await ytl(xky, "fdinp", "请填写手机号", qqq.data.data.mobile);
        break;
      } else if (qqwe3.uiObjects != "" && qqwe1.uiObjects == "") {
        await ytl(xky, "ysclick", "登录", 1);
        await xky.sleep(2000);
        break;
      } else if (qqwe4.uiObjects != "" && qqwe1.uiObjects == "") {
        await ytl(xky, "ysclick", "下一步", 1);
        await xky.sleep(2000);
        break;
      } else if (qqwe5.uiObjects != "") {
        await ytl(xky, "hdpt", "tcaptcha_drag_thumb");
        await xky.sleep(3000);
        break;
      } else if (qqwe6.uiObjects != "") {
        await ytl(xky, "ysclick", "开始验证 ");
        break;
      } else if (qqwe7.uiObjects != "") {
        await ytl(xky, "ysclick", "是");
        while (1) {
          await xky.sleep(8 * 1000);
          let hx1 = await ytl(xky, "hqxx", "应急联系人", 1);//限时查找
          let hx2 = await ytl(xky, "hqxx", "com.tencent.mm:id/qr", 1);//限时查找
          if (hx1) {
            await ytl(xky, "gncz", "com.tencent.mm:id/qr", "返回");//功能(如前进,返回,后退等) 
            break;
          } else if (hx2) {
            break;
          }
...
async function userError(xky, element) {
  ...
  while (1) {
    ...
    if (useTime > 18000) {
      ...
      //登陆状态异常
      let denglu = await ytl(xky, "hqxx", "登录", 1);//限时查找
      if (denglu) {
        return 5;
      }
      //账号异常
      ...
      if (tisa.uiObjects != "") {
        await ytl(xky, "ysclick", "确定");
        return 5;
      }

      let qq1 = await ytl(xky, "hqxx", "请填写微信密码", 1);//限时查找
      if (qq1) {
        return 5;
      }
...
//阅读测试模块
async function ReadTest(xky, args, xutil) {
  let readArr = args.read_gzh.split("\n")
  await elementClick(xky, "我");
  let MyNameArr = await getElementMassage(xky, "com.tencent.mm:id/a63");
  let MyName = MyNameArr["0"].text
  await elementClick(xky, "通讯录");
  await xky.sleep(3 * 1000);
  await rollFindElement(xky, MyName, "xia", 2);
  await elementClick(xky, MyName);
  await elementClick(xky, "发消息");
  for (let i = 0; i < readArr.length; i++) {
    await xky.sleep(2000)
    let dourl = readArr[i];
    await findElementIput(xky, "android.widget.EditText", dourl);
    await elementClick(xky, "发送");
    await xky.sleep(3 * 1000);
    let MySayArr = await getElementMassage(xky, "com.tencent.mm:id/nu");
    await xky.click(MySayArr[MySayArr.length - 1].x, MySayArr[MySayArr.length - 1].y - 0.01);
    await xky.sleep(1000)
    let MySayArr1 = await getElementMassage(xky, "com.tencent.mm:id/nu", 1);
    if (MySayArr1) {
      await xky.click(MySayArr[MySayArr.length - 1].x, MySayArr[MySayArr.length - 1].y - 0.02);
      await xky.sleep(1000)
      let MySayArr2 = await getElementMassage(xky, "com.tencent.mm:id/nu", 1);
      if (MySayArr2) {
        await xky.click(MySayArr[MySayArr.length - 1].x, MySayArr[MySayArr.length - 1].y + 0.01);
      }
    }
    //统计时间
    let startTime = new Date();
    await xky.sleep(6000);
    while (1) {
      let duwan = await rollFindElement(xky, "js_read_area3", "xia", 6, 8);
      if (duwan == "查找超时") {
        let MySayArr5 = await getElementMassage(xky, "网页无法打开", 1);
        if (MySayArr5) {
          await elementClick(xky, "更多");
          await elementClick(xky, "刷新");
          await xky.sleep(3000);
        }
        let MySayArr6 = await getElementMassage(xky, "轻触屏幕重新加载", 1);
        if (MySayArr6) {
          await xky.click(0.5, 0.5);
          await xky.click(0.5, 0.5);
        }
      } else {
        break;
