async function main(xky, args, xutil) {
    for (let i = 0; i < args.main_check.length; i++) {
        await xky.sleep(2000);
        await xky.toast(`开始${args.main_check[i]}`);
        switch (args.main_check[i]) {
            case "配置VPN":
                await SetVpn(xky);
                break;
            case "设置定位开发保留":
                await SetPositionOpenBluetooth(xky);
                break;
            case "配置触动精灵环境":
                await configChuDong(xky, args, xutil);
                break;
            case "安装微信":
                await InstallWecat(xky, args, xutil);
                break;
            case "安装抖音":
                await installDouyin(xky, args, xutil);
                break;
            case "安装微博":
                await installWeiBo(xky, args, xutil);
                break;
            case "安装快手":
                await installKuaiShou(xky, args, xutil);
                break;
            case "安装陌陌":
                await installMoMo(xky, args, xutil);
                break;
            case "安装探探":
                await installTanTan(xky, args, xutil);
                break;
            case "安装今日头条":
                await installJinRiTouTiao(xky, args, xutil);
                break;
            case "安装积木":
                await installJiMu(xky, args, xutil);
                break;
            case "安装qq邮箱":
                await installQQYouXiang(xky, args, xutil);
                break;
            case "安装QQ":
                await installQQ(xky, args, xutil);
                break;
            case "安装全名K歌":
                await installQuanMinKGe(xky, args, xutil);
                break;
        }
    }
}
...
//配置VPN
async function SetVpn(xky) {
    await xky.adbCommand('shell mkdir /data/redsocks');
    await xky.adbCommand('shell chmod -R 777 /data/redsocks');
    await xky.adbCommand('shell curl -o /data/redsocks/redsocks http://119.23.32.104:91/file/redsocks');
    await xky.adbCommand('shell curl -o /data/redsocks/redsocks.conf http://119.23.32.104:91/file/redsocks.conf');
    await xky.adbCommand('shell curl -o /data/redsocks/redsocks.sh http://119.23.32.104:91/file/redsocks.sh');
    await xky.adbCommand('shell curl -o /data/redsocks/busybox http://119.23.32.104:91/file/busybox');
    await xky.adbCommand('shell curl -o /data/redsocks/redsocks.conf.bak http://119.23.32.104:91/file/redsocks.conf.bak');
    await xky.adbCommand('shell chmod -R 777 /data/redsocks');
}

//设置定位开发保留
async function SetPositionOpenBluetooth(xky) {
    await xky.restartApp('com.android.settings');
    await xky.sleep(3 * 1000);
    await rollFindElement(xky, "位置信息", "xia", 8);//已num格的速度查找
    await elementClick(xky, "位置信息");//必须点
    await elementClick(xky, "模式");//必须点
    await elementClick(xky, "高精确度");//必须点
...
async function closeNoSave(xky) {
    ...
    if (!bb) {
        ...
        await elementClick(xky, "关于手机");//必须点
        await rollFindElement(xky, "版本号", "xia", 5);//已num格的速度查找
        await elementClick(xky, "版本号");//必须点
        await elementClick(xky, "版本号");//必须点
        await elementClick(xky, "版本号");//必须点
        await elementClick(xky, "版本号");//必须点
        await elementClick(xky, "版本号");//必须点
        ...
        await elementClick(xky, "版本号");//必须点
        await elementClick(xky, "版本号");//必须点
        await elementClick(xky, "菜单");//必须点
        await elementClick(xky, "开发者选项");//必须点
    }
    await xky.sleep(2 * 1000);
...
//配置触动精灵环境
async function configChuDong(xky, args, xutil) {
    //结束APP
    await xky.killApp('com.touchsprite.android');
    await xky.sleep(2000);
    //清空APP
    await xky.clearApp('com.touchsprite.android');
    await xky.sleep(2000);
    //打开触动权限
    await xky.adbCommand('shell cp /system/xbin/sv /system/xbin/su');
    //安装触动精灵
    await xky.installApkFromCloud('test0001/TouchSprite_v2.4.0_signed.apk');
    await xky.sleep(2 * 1000);
    await xky.installApkFromCloud('test0001/TSVPN101.apk');
    await xky.sleep(2 * 1000);
    await xky.downloadFile("chudong/start.lua", "/storage/emulated/0/TouchSprite/lua/start.lua");
    await xky.downloadFile("chudong/stop.lua", "/storage/emulated/0/TouchSprite/lua/stop.lua");
    await xky.sleep(2 * 1000);
    await setChuDong(xky, args, xutil, 2, "");//1开启,2关闭,3设置地区,4设置账号
}

//安装微信
async function InstallWecat(xky, args, xutil) {
    await xky.installApkFromUrl('http://dldir1.qq.com/weixin/android/weixin703android1400.apk');
}

//安装抖音
async function installDouyin(xky, args, xutil) {
    await xky.installApkFromUrl("http://app.mi.com/download/431355?id=com.ss.android.ugc.aweme&ref=appstore.mobile_download&nonce=7017895517738497817%3A25805500&appClientId=2882303761517485445appSignature=kBF80u2NEzYtJZ1mN5DOHWgpa7KaR_5Ni3_D5kTWXWE");
}

//安装微博
async function installWeiBo(xky, args, xutil) {
    await xky.installApkFromUrl("http://qunying.jb51.net:81/201307/tools/XL.weibo_jb51.net.apk");
}

//安装快手
async function installKuaiShou(xky, args, xutil) {
    await xky.installApkFromUrl("https://js.a.kspkg.com/bs2/fes/kwai-android-generic-gifmakerrelease-6.0.1.7800_b7a7f0.apk");
}

//安装陌陌
async function installMoMo(xky, args, xutil) {
    await xky.installApkFromUrl("http://server.m.pp.cn/download/apk?appId=280155&productId=2008&ch_src=pp_uc_appdownload&ei=d3MAA7wQ67rpmkV93Wex2HxDkNk%3D%0D%0A&ai=d3MAA4vlVAyrgsa6NBWGNZC2QP0%3D%0D%0A&uuid=d3MAA5Bv%2BQqbsmWSgeRTcKem9W6YQ8V%2BEnYHQjt%2FecJtoJvddCGRaAdPqA%2FMSumWsQdpcQ%3D%3D%0D%0A&uc_param_str=frvecpeimemintnidnut");
}

//安装探探
async function installTanTan(xky, args, xutil) {
    await xky.installApkFromUrl("https://apk-ssl.tancdn.com/3.3.6.1_247/%E6%8E%A2%E6%8E%A2.apk");
}

//安装今日头条
async function installJinRiTouTiao(xky, args, xutil) {
    await xky.installApkFromUrl("http://server.m.pp.cn/download/apk?appId=35609&productId=2008&ch_src=pp_uc_appdownload&ei=d3MAA7wQ67rpmkV93Wex2HxDkNk%3D%0D%0A&ai=d3MAA4vlVAyrgsa6NBWGNZC2QP0%3D%0D%0A&uuid=d3MAA5Bv%2BQqbsmWSgeRTcKem9W6YQ8V%2BEnYHQjt%2FecJtoJvddCGRaAdPqA%2FMSumWsQdpcQ%3D%3D%0D%0A&ut=d3MAAySy%2BDbG4x%2FKdX8PVzjHE7Opr%2F6eNtkSwtqyjxq%2B190q%0D%0A&uc_param_str=frvecpeimemintnidnut");
}

//安装积木
async function installJiMu(xky, args, xutil) {
    await xky.installApkFromUrl("http://b.downcc.com/apk2/jimuv1.3.2_downcc.com.apk");
}

//安装qq邮箱
async function installQQYouXiang(xky, args, xutil) {
    await xky.installApkFromUrl("http://dldir1.qq.com/foxmail/qqmail_android_5.5.6.10134093.2399_0.apk");
}

//安装qq
async function installQQ(xky, args, xutil) {
    await xky.installApkFromUrl("https://qd.myapp.com/myapp/qqteam/AndroidQQ/mobileqq_android.apk");
}

//安装全名K歌
async function installQuanMinKGe(xky, args, xutil) {
    await xky.installApkFromUrl("http://server.m.pp.cn/download/apk?appId=6514875&productId=2008&ch_src=pp_uc_appdownload&ei=d3MAA7wQ67rpmkV93Wex2HxDkNk%3D%0D%0A&ai=d3MAA4vlVAyrgsa6NBWGNZC2QP0%3D%0D%0A&uuid=d3MAA5Bv%2BQqbsmWSgeRTcKem9W6YQ8V%2BEnYHQjt%2FecJtoJvddCGRaAdPqA%2FMSumWsQdpcQ%3D%3D%0D%0A&ut=d3MAAySy%2BDbG4x%2FKdX8PVzjHE7Opr%2F6eNtkSwtqyjxq%2B190q%0D%0A&uc_param_str=frvecpeimemintnidnut");
}
