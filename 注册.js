async function main(xky, args, xutil) {
  xky.log(`提示1:如果注册完成后程序错误或网络原因停止了,请点击强制保存模块`)
  xky.log(`提示2:如果注册完成后程序错误或网络原因停止了,请点击强制保存模块`)
  //配置微信权限和关闭活动保留
  ...
  //设置代理地区代号
  await setChuDong(xky, args, xutil, 4, args.ka_diqu);
  await setChuDong(xky, args, xutil, 3, args.ka_diqu);
  let creatNums = args.creatNums;

  while (1) {
    //开始注册
    let vpnUseTime = await cardShop(xky, args, xutil);
    await xky.sleep(3 * 1000);
    for (let i = 0; i < args.zhece_gongneng.length; i++) {
      await xky.toast(`注册完成正在执行${args.zhece_gongneng[i]}`);
      switch (args.zhece_gongneng[i]) {
        case "小红点清理":
          await xky.sleep(2 * 1000);
          await clearRed(xky, args, xutil);
          break;
      }
    }
    let vpnEndTime = new Date();
    let vpnUseTimeArr = vpnEndTime - vpnUseTime;
    xky.log(`本次注册VPN总耗时${vpnUseTimeArr}`)
    xky.log(`本次注册VPN总耗时${vpnUseTimeArr}`)
    xky.log(`本次注册VPN总耗时${vpnUseTimeArr}`)
    await xky.sleep(5 * 1000);
    await xky.toast("正在上传插槽信息");
    await xky.killApp('com.tencent.mm');
    await setChuDong(xky, args, xutil, 2, args.ka_diqu);
    //上传插槽信息
    ...
    await xky.sleep(2 * 1000);
    await xky.toast("开始注册下一个");
    creatNums--;
    if (creatNums < 1) {
      xky.log(`已经完成${args.creatNums}个账号的注册`)
      xky.log(`已经完成${args.creatNums}个账号的注册`)
      return;
    }
...
//处理获取验证信息错误
async function verifyWrong(xky) {
  let xxcw = await xky.findUiObjects('获取验证信息系统错误', { regex: true, timeout: 2000 });
  if (xxcw.uiObjects != "") {
    await elementClick(xky, "返回");
    await elementClick(xky, "注册", 3);
    let hds = await elementClick(xky, "开始", 3);
    if (hds) {
...
async function setPassWord() {
  let len3 = 3;
  let chars1 = "abcdefhijkmnprstwxyz";
  let maxPos = chars1.length;
  let pwd1 = "";
  for (let i = 0; i < len3; i++) {
    pwd1 += chars1.charAt(Math.floor(Math.random() * maxPos));
  }
  let passWord1 = pwd1;
  let len5 = 5;
  let chars2 = "1234567890";
  let maxPos2 = chars2.length;
  let pwd2 = "";
  for (let i = 0; i < len5; i++) {
    pwd2 += chars2.charAt(Math.floor(Math.random() * maxPos2));
  }
  let passWord2 = pwd2;
  //保存密码
...
async function cardShop(xky, args, xutil) {
  //卡商信息
  let [ssid, userApi] = ["329", "api-169610-r9TSWP6"]
  if (args.user_Api == "陆哥的") {
    userApi = "api-119772-r2QPQyS"
  }
  let [userPas, userToken, phone] = ["hl1410", "", ""]
  //循环注册
  while (1) {
    //登陆卡商
    await xky.toast("开始获取手机号码");
    while (1) {
      let login = await xutil.http.get(
        `http://huoyun888.cn/api/do.php?action=loginIn&name=${userApi}&password=${userPas}`
      );
      if (login.data.indexOf("1|") >= 0) {
        userToken = login.data.split("|")[1];
        break;
      }
      await xky.sleep(2 * 1000);
    }
    ...
    //获取卡号
    while (1) {
      let phoneGet = await xutil.http.get(
        `http://huoyun888.cn/api/do.php?action=getPhone&sid=${ssid}&token=${userToken}`
      );
      if (phoneGet.data.indexOf("1|") >= 0) {
        xky.log(phoneGet)
        phone = phoneGet.data.split("|")[1];
        break;
      }
      aaaa++;
      if (aaaa > 2) {
        xky.log(phoneGet)
        if (phoneGet.data.indexOf("余额不足") >= 0) {
          xky.log("*************火云余额不足,请及时充值*************");
          xky.log("*************火云余额不足,请及时充值*************");
        }
      }
      await xky.sleep(5 * 1000);
    }
    await xky.toast("开始配置环境并注册");
    await xky.clearContacts();
    let phonePeopleArr = await toLeadInsCon(xky, xutil, 8);
    await setChuDong(xky, args, xutil, 1, args.ka_diqu);
    let vpnUseTime = new Date();
    await setLocations(xky);
    //创建插槽
    let ccCode = await creatSlot(xky, "com.tencent.mm", 18);
    let nicker = await getNikeName(xky, xutil, "cn");
    let nickName = nicker.data.data.nickname;
    //随机生成密码
    let passWord = await setPassWord()
    xky.log("***************");
    xky.log("手机号:" + phone)
    xky.log("密码:" + passWord);
    xky.log("插槽码:" + ccCode);
    ...
    await findElementIput(xky, "例如：陈晨", nickName);
    await findElementIput(xky, "请填写手机号", phone);
    await findElementIput(xky, "填写密码", passWord);
    await elementClick(xky, "注册");
    await regularClick(xky, "我已阅读并同意上述条款");
    await regularClick(xky, "下一步");
    await xky.sleep(3 * 1000);
    await regularClick(xky, "开始");
    await slideWecatJigsaw(xky, "tcaptcha_drag_thumb");
    await xky.sleep(3 * 1000);
    await verifyWrong(xky);
    //短信注册或者二维码注册
    let SimOrCode = "";
    while (1) {
      let dxyzmsArr1 = await getElementMassage(xky, "让用户用微信扫描下面的二维码", 1);
      let dxyzmsArr2 = await getElementMassage(xky, "发送短信验证", 1);
      if (dxyzmsArr1) {
        SimOrCode = 1;
        break;
      }
    ...
    if (SimOrCode == 1) {
      let [ymuserKey, ymUrl, orderUrl, signUrl] = ["a6e98f0c6f4e4ce08f22ca0ee97e6d06", "http://ymka.ymassist.com/assist/api/order/submit", "http://ymka.ymassist.com/assist/api/order/query", "http://ymka.ymassist.com/assist/api/order/mark"]
      let startTime = new Date();
      let [imgName, orderId, ewms] = ["", "", 0]
      while (1) {
        await xky.adbShell(" rm /sdcard/tencent/MicroMsg/WeiXin/*.*");
        await xky.sleep(2 * 1000);
        await xky.mousedown(0.4, 0.63);
        await elementClick(xky, "保存到手机");
        await xky.sleep(3 * 1000);
        let qqq1 = await xky.adbCommand(' ls  /sdcard/tencent/MicroMsg/WeiXin/');
        let qqq2 = qqq1.result;
        let imgarr = qqq2.split(" ");
        let imggs = imgarr[imgarr.length - 1];
        if (imggs.indexOf("mmexport") >= 0) {
          imgName = imggs.replace("\n", "")
          xky.log(imgName)
        } else {
          continue;
        }
        await xky.sleep(1 * 1000);
        //上传到服务器
        await xky.adbShell(`curl ftp://238z1w6709.qicp.vip:8021/${imgName} -u admin:123456 -T /sdcard/tencent/MicroMsg/WeiXin/${imgName}`);
        await xky.sleep(5000)
        //获取解析地址
        let qrCodeUrl = await xutil.http.get(`http://238z1w6709.qicp.vip:8090/noticecl/decoderQRCode?filename=${imgName}`)
        xky.log(`${qrCodeUrl.data}`)
        await xky.sleep(2000)
        //发送二维码
        let ppp = await xutil.http.get(`${ymUrl}?userKey=${ymuserKey}&qrCodeUrl=${qrCodeUrl.data}`)
        xky.log(ppp)
        if (ppp.data.msg == "请提交正确格式的二维码") {
          ewms++;
          if (ewms > 5) {
            let lahei = await xutil.http.get(
              `http://huoyun888.cn/api/do.php?action=addBlacklist&sid=${ssid}&phone=${phone}&token=${userToken}`
            );
            xky.log("******已经拉黑号码******")
            xky.log(lahei)
            xky.log("******已经拉黑号码******")
        ...
        orderId = ppp.data.object.orderId;
        xky.log(orderId)
        await xky.sleep(5000)
        break;
      }
      ...
      while (1) {
        await verifyWrong(xky);
        let smright = await xky.findUiObjects('返回注册流程', { regex: true, timeout: 2000 });
        let xianzhi = await xky.findUiObjects('已超出限制', { regex: true, timeout: 2000 });
        if (smright.uiObjects != "") {
          //如果成功了直接标记订单完成
          let orderGood = await xutil.http.get(`${signUrl}?userKey=${ymuserKey}&orderId=${orderId}&status=3`)
          xky.log(orderGood)
          //删除服务器二维码
          await xutil.http.get(`http://238z1w6709.qicp.vip:8090/noticecl/deleteImg?file=${imgName}`)
          break;
        } else {
          await verifyWrong(xky);
          await xky.sleep(20000)
          ...
          if (yyy % 2 == 0) {
            //查询状态
            let orderForm = await xutil.http.get(`${orderUrl}?userKey=${ymuserKey}&orderId=${orderId}`)
            xky.log(orderForm.data.object)
          }

          if (useTime > 320000 || xianzhi.uiObjects != "") {
            //超时标记失败
            let orderGood = await xutil.http.get(`${signUrl}?userKey=${ymuserKey}&orderId=${orderId}&status=6`)
            xky.log(orderGood)
      ...
      await verifyWrong(xky);
      await regularClick(xky, "返回注册流程");
    }
    //获取发送内容
    await xky.sleep(2 * 1000);
    await getElementMassage(xky, "发送短信验证");
    let sendArr = await getElementMassage(xky, "com.tencent.mm:id/du9");
    let toNumArr = await getElementMassage(xky, "com.tencent.mm:id/du_");
    let send = sendArr[0].text.substring(3);
    let toNum = toNumArr[0].text.substring(2);
    //发送短信
    let sendMessageNums = 0;
    while (1) {
      await xky.sleep(2 * 1000);
      let sendMessage = await xutil.http.get(
        `http://huoyun888.cn/api/do.php?action=putSentMessage&phone=${phone}&sid=${ssid}&message=${send}&recvPhone=${toNum}&token=${userToken}`
      );
      xky.log(sendMessage)
      if (sendMessage.data.indexOf("1|") >= 0) {
        xky.log(sendMessage.data)
        await xky.sleep(8 * 1000);
        await elementClick(xky, "已发送短信，下一步");
        break;
      } else {
        sendMessageNums++;
        if (sendMessageNums > 3) {
          xky.toast(`******请使用号码${phone}:手动发送短信${send}到${toNum},并手动点击下一步,并手动点击下一步******`)
          xky.log(`******请使用号码${phone}:手动发送短信${send}到${toNum},并手动点击下一步,并手动点击下一步******`)
          xky.log(`******请使用号码${phone}:手动发送短信${send}到${toNum},并手动点击下一步,并手动点击下一步******`)
          await xky.sleep(5 * 1000);
          xky.toast(`******请使用号码${phone}:手动发送短信${send}到${toNum},并手动点击下一步,并手动点击下一步******`)
          break;
    ...
    await elementClick(xky, "不是我的，继续注册", 16);
    await xky.sleep(3 * 1000);
    let chongfu = await xky.findUiObjects('频繁重复注册', { regex: true, timeout: 2000 });
    let chongfu2 = await xky.findUiObjects('当前手机号', { regex: true, timeout: 2000 });
    if (chongfu.uiObjects != "" || chongfu2.uiObjects != "") {
      await xky.toast("删除本地插槽结束微信");
      await xky.killApp('com.tencent.mm');
      await setChuDong(xky, args, xutil, 2, "绵阳");
      await xky.sleep(2 * 1000);
    ...
    if (isError == 5) {
      ...
      xky.log("<<<<<<正在处理异常账号>>>>>>>");
      await xky.sleep(5000);
      var isGood = await repairError(xky, xutil, qqs);
      if (isGood == 1) {
        //运行正常
        xky.log(">>>>>>>处理完毕,账号正常<<<<<<");
      } else {
    ...
    xky.log("一次检测通过,开始二次检测");
    //二次检测
    let knowGood = "";
    let gggg = await getElementMassage(xky, "腾讯新闻", 8);
    if (!gggg) {
      while (1) {
        let isNoNet = await elementClick(xky, "加载联系人失败，点击重试", 1);
        let isNoNet1 = await elementClick(xky, "当前网络不可用，请点击检查手机网络设置", 1);
        if (isNoNet || isNoNet1) {
          await xky.sleep(15 * 1000);
          await elementClick(xky, "完成", 1);
        } else {
          await elementClick(xky, "确定");
          knowGood = await repairError(xky, xutil, qqs);
          break;
        }
      }
    }
    ...
    xky.log("检测完成,账号正常");
    await getElementMassage(xky, "com.tencent.mm:id/d7b");
    await elementClick(xky, "我");
    await elementClick(xky, "com.tencent.mm:id/py");
    await xky.sleep(3 * 1000);
    let blStat = await getElementMassage(xky, "android:id/summary");
    await xky.sleep(1 * 1000);
    let ysbh = blStat[1].text;
    let extendData = {
      cCode:ccCode,
      addressbook:phonePeopleArr
    }
    let saveUser = await saveData(xky, xutil, args.AppID, "", args.depot, phone, passWord, phone, nickName, ysbh, args.ka_diqu,extendData);
    xky.log(saveUser);
    await xky.sleep(3 * 1000);
    await findUseFunction(xky, "com.tencent.mm:id/d7b", "返回");
    return vpnUseTime;
...
//清除红点(注册)
async function clearRed(xky, args, xutil) {
  ...
  while (1) {
    ...
    await elementClick(xky, "com.tencent.mm:id/bta");
    await xky.sleep(3 * 1000);
    let meiyoupy = await getElementMassage(xky, "还没有推荐的朋友", 1);
    if (meiyoupy) {
      await findUseFunction(xky, "com.tencent.mm:id/d7b", "返回");
      break;
    }
    ...
    await xky.sleep(3 * 1000);
    let faxiaoxi = await getElementMassage(xky, "发消息", 1);
    if (faxiaoxi) {
      await elementClick(xky, "返回");
    }
    await xky.sleep(2 * 1000);
  ...
  await findUseFunction(xky, "com.tencent.mm:id/d7b", "返回");
  await elementClick(xky, "我");
  let shortMv = await elementClick(xky, "com.tencent.mm:id/k_", 3);
  if (shortMv) {
    await elementClick(xky, "我知道了");
    await elementClick(xky, "关闭");
  }
  let setMsg = await elementClick(xky, "com.tencent.mm:id/akk", 3);
  if (setMsg) {
    await elementClick(xky, "NEW");
    await elementClick(xky, "NEW");
    await xky.sleep(2 * 1000);
    await findUseFunction(xky, "com.tencent.mm:id/d7b", "返回");
  }
...
//处理异常号
async function repairError(xky, xutil, qqq) {
  while (1) {
    while (1) {
      ...
      if (qqwe1.uiObjects != "") {
        await findElementIput(xky, "请填写微信密码", qqq.password);
        break;
      } else if (qqwe2.uiObjects != "") {
        await findElementIput(xky, "请填写手机号", qqq.mobile);
        break;
