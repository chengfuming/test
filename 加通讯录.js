...
async function examineAddTxl(xky, args, xutil, qqq) {
  ...
  for (let i = 0; i < sss.data.data.length; i++) {
    ...
    let isAdd = await getElementMassage(xky, "com.tencent.mm:id/q0", 3);//限时查找
    if (isAdd) {
      await contactFinish(xky, xutil, qqq.data.data.id, 2, sss.data.data[i].number)//type:1取备注编号 2完成添加 3拉黑,number:获取的号码
      await elementClick(xky, "com.tencent.mm:id/q0");//必须点
      await coordinatesClick(xky,"com.tencent.mm:id/nu", 0.08, 0.22);
      await elementClick(xky, "设置备注和标签");//必须点
      await elementClick(xky, "com.tencent.mm:id/b4w");//必须点
      await findElementIput(xky,"com.tencent.mm:id/b4v", "");
      await elementClick(xky, "完成");//必须点
      await findUseFunction(xky, "com.tencent.mm:id/ki", "返回");//方法(如前进,返回,后退等)
    }
    await xky.sleep(1000);
  }
  await findUseFunction(xky, "com.tencent.mm:id/d7b", "返回");//方法(如前进,返回,后退等)
...
async function AddTxlFriend(xky, args, xutil, qqq) {
  ...
  while (1) {
    ...
    if (isTxl) {
      ...
      await elementClick(xky, "发送",5);//必须点
      addNum--;
      if (addNum < 1) {
        await findUseFunction(xky, "com.tencent.mm:id/d7b", "返回");//方法(如前进,返回,后退等)
        return 1;
...
//正则点击
async function regularClick(xky, element) {
  while (1) {
